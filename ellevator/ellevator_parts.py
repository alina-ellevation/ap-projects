class Building():
    def __init__(self):
        '''A building with ELLevator service.
    ...
    def dispatch(self):
        while len(self.stop_queue) == 0:
            ...
            if self.is_going_up is None:
                print('Cabin {} is standing by - no requests left to serve.'.format(self.id))
                break
            ...
            while len(self.stop_queue) > 0:
                print('Floor(s) served next: {}'.format(self.stop_queue))
                ...
                for p in passengers_to_enter:
                    if len(self.passengers) < self.passenger_max:
                        ...
                        p.select_floor()
                        self.stop_queue = self.order_floors(floors_to_order=self.stop_queue,
                                                            floor_order=self.served_floors,
                                                            lowest_to_highest=self.is_going_up)
                    else:
...
class Passenger():
    def __init__(self, name, building, current_floor, destination_floor):
        ...
        Attributes:
            ...
            is_going_up (bool or None): whether the person
        '''
        self.name = name
        self.building = building
        self.current_floor = current_floor
